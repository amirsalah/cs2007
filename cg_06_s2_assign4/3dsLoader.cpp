...
void ReadColourChunk(materialInfo *material, chunk *cChunk, int type);
...
void ResizeObjArray(model3D *model){
	...
	tempNumObjs = (int)(tempNumObjs*1.5);
...
void ResizeMatArray(model3D *model){
	...
	tempNumMats = (int)(tempNumMats*1.5);
...
void ProcessMaterialChunk(model3D *model, chunk *previousChunk){
	...
	while (previousChunk->bytesRead < previousChunk->length)
	{
		...
		case MATAMBIENT:
			ReadColourChunk(&(model->materials[model->numMaterials - 1]), currentChunk, 0);
			break;
		
		case MATDIFFUSE:
			ReadColourChunk(&(model->materials[model->numMaterials - 1]), currentChunk, 1);
			break;

		case MATSPECULAR:
			ReadColourChunk(&(model->materials[model->numMaterials - 1]), currentChunk, 2);
			break;

		case MATSHININESS:
			ReadColourChunk(&(model->materials[model->numMaterials - 1]), currentChunk, 3);
			break;
...
void ReadColourChunk(materialInfo *material, chunk *cChunk, int type){
	...
	switch (type)
		{
			case 0:
				tempChunk->bytesRead += fread(material->ambColour, 1, tempChunk->length - tempChunk->bytesRead, filePointer);
				break;
			case 1:
				tempChunk->bytesRead += fread(material->diffColour, 1, tempChunk->length - tempChunk->bytesRead, filePointer);
				break;
			case 2:
				tempChunk->bytesRead += fread(material->specColour, 1, tempChunk->length - tempChunk->bytesRead, filePointer);
				break;
			case 3:
				tempChunk->bytesRead += fread(&material->shine, 1, tempChunk->length - tempChunk->bytesRead, filePointer);
				break;
		}
